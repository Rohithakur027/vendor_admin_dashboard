...
function buildRenderers(
  ...
  return {
    tripId: {
      ...
      skeleton: () => (<div className="space-y-2"><Skeleton className="h-3.5 w-16" /><Skeleton className="h-4 w-14 rounded" /></div>),
      csv:      (b: Booking) => `${b.bookingRef ?? ""} · Scheduled`,
    },
    route: {
      header:   () => "ROUTE",
      body:     (b: Booking) => (
    ...
    status: {
      ...
      csv:      (b: Booking) => b.driverId ? "Driver Assigned" : "Awaiting Driver",
    },
    pickupTime: {
      header:   () => "PICKUP TIME",
      body:     (b: Booking) => {
